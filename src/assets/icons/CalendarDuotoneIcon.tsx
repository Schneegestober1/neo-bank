import { forwardRef, type Ref, type SVGProps } from 'react'

const CalendarDuotoneIcon = forwardRef<SVGSVGElement, SVGProps<SVGSVGElement>>(
  (props, ref: Ref<SVGSVGElement>) => (
    <svg
      width="40"
      height="40"
      viewBox="0 0 40 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      ref={ref}
      {...props}
    >
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12.6667 6.96678C12.6667 6.82529 12.6667 6.75454 12.6227 6.7106C12.5787 6.66666 12.5079 6.66673 12.3662 6.66689C9.01367 6.67056 7.27559 6.73422 6.17157 7.83824C5 9.00981 5 10.8954 5 14.6667V25.3333C5 29.1046 5 30.9902 6.17157 32.1618C7.34315 33.3333 9.22876 33.3333 13 33.3333H27C30.7712 33.3333 32.6569 33.3333 33.8284 32.1618C35 30.9902 35 29.1046 35 25.3333V14.6667C35 10.8954 35 9.00981 33.8284 7.83824C32.7244 6.73422 30.9863 6.67056 27.6338 6.66689C27.4922 6.66673 27.4213 6.66666 27.3773 6.7106C27.3333 6.75454 27.3333 6.82529 27.3333 6.96678V10.8333C27.3333 11.6618 26.6618 12.3333 25.8333 12.3333C25.0049 12.3333 24.3333 11.6618 24.3333 10.8333V6.96666C24.3333 6.82524 24.3333 6.75453 24.2894 6.7106C24.2455 6.66666 24.1748 6.66666 24.0333 6.66666H15.9667C15.8252 6.66666 15.7545 6.66666 15.7106 6.7106C15.6667 6.75453 15.6667 6.82524 15.6667 6.96666V10.8333C15.6667 11.6618 14.9951 12.3333 14.1667 12.3333C13.3382 12.3333 12.6667 11.6618 12.6667 10.8333V6.96678Z"
        fill="#2A4157"
        fillOpacity="0.24"
      />
      <path d="M14.1667 4.16666L14.1667 10.8333" stroke="#222222" strokeLinecap="round" />
      <path d="M25.8332 4.16666L25.8333 10.8333" stroke="#222222" strokeLinecap="round" />
      <circle cx="12" cy="16" r="1" fill="#222222" />
      <circle cx="17" cy="16" r="1" fill="#222222" />
      <circle cx="23" cy="16" r="1" fill="#222222" />
      <circle cx="28" cy="16" r="1" fill="#222222" />
      <circle cx="12" cy="22" r="1" fill="#222222" />
      <circle cx="17" cy="22" r="1" fill="#222222" />
      <circle cx="23" cy="22" r="1" fill="#222222" />
      <circle cx="28" cy="22" r="1" fill="#222222" />
      <circle cx="12" cy="28" r="1" fill="#222222" />
      <circle cx="17" cy="28" r="1" fill="#222222" />
      <circle cx="23" cy="28" r="1" fill="#222222" />
      <circle cx="28" cy="28" r="1" fill="#222222" />
    </svg>
  )
)

export default CalendarDuotoneIcon
